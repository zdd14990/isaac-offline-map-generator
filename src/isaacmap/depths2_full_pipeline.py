"""Canonical Depths-II accepted-layout research composition.

All geometry and selection work is delegated to existing clean leaves.  This
module owns only the Stage-6 replay/attempt transaction and does not perform
...
from .floor_init import derive_depths2_topology_inputs
from .generation_state import (
    CanonicalDepths2Profile,
...
DEPTHS2_LAYOUT_STATUS = "DEPTHS2_FULL_LAYOUT_PIPELINE_CONFIRMED_BINARY"
EXTERNAL_VALIDATION = "NOT_EXTERNALLY_VALIDATED_GAMEPLAY"
LEVEL_SHIFT_INDEX = 35


@dataclass(frozen=True)
class Depths2FullResult:
    completed: bool
    ...
    difficulty: str
    stage_seed: int
    replayed_depths1: Depths1FullResult
...
def generate_depths2_full(
    depths1_result: Depths1FullResult,
    ...
    *,
    ...
    """Generate Depths II from a successful Depths I result and explicit entry state.

    The entry state must be a CanonicalRunGenerationSnapshot with next_level_stage=6
    and completed_level_stage=5, typically from Depths1LifecycleResult.next_run_state.
    The Depths BossPool (index 7) RNG state is inherited (advanced) from Depths I.
    """
    snapshot = entry_state_snapshot
    if snapshot.next_level_stage != 6 or snapshot.completed_level_stage != 5:
        raise ValueError(
            f"Depths II entry requires snapshot with "
            f"next_level_stage=6, completed_level_stage=5; "
            f"got next={snapshot.next_level_stage}, completed={snapshot.completed_level_stage}"
        )

    start_seed = snapshot.game_start_seed

    inputs = derive_depths2_topology_inputs(start_seed, depths1_result.difficulty)
    profile = CanonicalDepths2Profile(
        depths1_result.difficulty,
    ...
    run_state = RunGenerationState(
        ...
        bosses,
        room_config,
        "depths1-success-tail-complete/depths2-entered",
    ...
    for attempt_number in range(1, max_attempts + 1):
                ...
                if not m6.completed:
                    ...
                    m7 = run_secret_to_ultra(
                        _m6_boundary(
                            ...
                            difficulty=depths1_result.difficulty,
        ...
        if late is not None and late.completed:
            ...
            run_state.current_stage_lifecycle = "depths2-full-layout-accepted"
            return Depths2FullResult(
                True,
                DEPTHS2_LAYOUT_STATUS,
                EXTERNAL_VALIDATION,
                start_seed,
                depths1_result.difficulty,
                inputs.stage_seed,
