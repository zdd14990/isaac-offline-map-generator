"""Mechanical Dross (2+) accepted-layout reference composition.

Independent orchestration reference for the Stage-2 ALT entry (stage_type 5,
rcs 28, slot 3).  It never calls a clean high-level pipeline.
"""
...
from .generation_state import CanonicalDrossProfile
...
def generate_dross_full_reference(
    start_seed: int,
    difficulty: str,
    *,
    dross_boss_entries: tuple[BossPoolEntryReference, ...],
    special_definitions: tuple[RoomDefinition, ...],
    dross_definitions: tuple[RoomDefinition, ...],
    max_attempts: int = 100,
) -> ReferenceBasement1FullResult:
    inputs = derive_alt_topology_inputs_reference(2, 5, start_seed, difficulty)
    ...
    entries = (
        ...
        + entries_from_definitions_reference(dross_definitions, stage=28)
    )
    weights = RoomConfigMutableStateReference()
    weights.reset_pool(entries)
    bosses = BossPoolRuntimeReference.fresh_alt(
        start_seed, DROSS_POOL_INDEX, dross_boss_entries
    )
    ...
    for number in range(1, max_attempts + 1):
        ...
        start_boss = bosses.pool_states[DROSS_POOL_INDEX]
            ...
            if not m5.completed:
                ...
                if not m6.completed:
                    ...
                    m6_attempt = ReferenceSecretAttempt(
                        ...
                        generator.rng.seed,
                        start_boss,
                        bosses.pool_states[DROSS_POOL_INDEX],
                        used_before,
