"""Mechanical Mausoleum1 (2+) accepted-layout reference composition.

Independent orchestration reference for the Stage-2 ALT entry (stage_type 5,
rcs 28, slot 6).  It never calls a clean high-level pipeline.
"""
...
def generate_mausoleum1_full_reference(
    start_seed: int,
    difficulty: str,
    *,
    mausoleum1_boss_entries: tuple[BossPoolEntryReference, ...],
    special_definitions: tuple[RoomDefinition, ...],
    mausoleum1_definitions: tuple[RoomDefinition, ...],
    max_attempts: int = 100,
) -> ReferenceBasement1FullResult:
    inputs = derive_alt_topology_inputs_reference(5, 4, start_seed, difficulty)
    ...
    entries = (
        ...
        + entries_from_definitions_reference(mausoleum1_definitions, stage=31)
    )
    ...
    for number in range(1, max_attempts + 1):
        ...
        start_boss = bosses.pool_states[MAUSOLEUM_POOL_INDEX]
            ...
            if not m5.completed:
                ...
                if not m6.completed:
                    ...
                    m6_attempt = ReferenceSecretAttempt(
                        ...
                        generator.rng.seed,
                        start_boss,
                        bosses.pool_states[MAUSOLEUM_POOL_INDEX],
                        used_before,
