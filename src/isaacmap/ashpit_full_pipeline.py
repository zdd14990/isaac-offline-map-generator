"""Canonical Ashpit (2+) accepted-layout research composition.

Stage-4 ALT entry: StageType 5, rcs 30, slot 5, pool 30.  The first-half
mandatory end room (0x0033C416..0x0033C4D1) fires on every Ashpit floor.
...
from .boss_pool import ASHPIT_POOL_INDEX, BossPoolEntry, BossPoolRuntimeState
from .floor_init import derive_alt_topology_inputs
from .generation_state import (
    CanonicalAshpitProfile,
...
ASHPIT_LAYOUT_STATUS = "ASHPIT_FULL_LAYOUT_PIPELINE_PARTIAL_BINARY"
EXTERNAL_VALIDATION = "NOT_EXTERNALLY_VALIDATED_GAMEPLAY"
LEVEL_SHIFT_INDEX = 35


def generate_ashpit_full(
    start_seed: int,
    difficulty: str,
    *,
    ...
    entries = (
        ...
        + entries_from_definitions(ashpit_definitions, stage=30)
    )
    run_state = RunGenerationState(
        start_seed,
        BossPoolRuntimeState.fresh_alt(start_seed, ASHPIT_POOL_INDEX, ashpit_boss_entries),
    ...
    for attempt_number in range(1, max_attempts + 1):
        ...
        start_boss = run_state.boss_pools.pool_rngs[ASHPIT_POOL_INDEX].seed
            ...
            if not m5.completed:
                ...
                if not m6.completed:
                    ...
                    m6_attempt = SecretGenerationAttemptResult(
                        ...
                        generator.rng.seed,
                        start_boss,
                        run_state.boss_pools.pool_rngs[ASHPIT_POOL_INDEX].seed,
                        used_before,
                    ...
                    m7 = run_secret_to_ultra(
                        _m6_boundary(
                            ...
                            level_rng=level_rng,
                            generator=generator,
                            boss_pool_index=ASHPIT_POOL_INDEX,
                        ),
                        entries=entries,
                    )
        ...
        if late is not None and late.completed:
            ...
            run_state.current_stage_lifecycle = "ashpit-full-layout-accepted"
            return Basement1FullResult(
                True,
                ASHPIT_LAYOUT_STATUS,
                EXTERNAL_VALIDATION,
                start_seed,
                difficulty,
                inputs.stage_seed,
