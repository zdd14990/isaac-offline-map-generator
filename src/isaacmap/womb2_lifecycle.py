"""Canonical Womb-II accepted layout plus successful lifecycle handoff.

The Stage-8 post-layout tail reuses the shared ``place_post_topology_rooms``
tail (RVA ``0x0033D1D6..0x0033D94B``).  Stage 8 (room-config stage 10) skips
...
from .womb1_lifecycle import Womb1LifecycleResult
from .womb2_full_pipeline import Womb2FullResult, generate_womb2_full


WOMB2_LIFECYCLE_STATUS = "WOMB2_CANONICAL_LIFECYCLE_PARTIAL_BINARY"
EXTERNAL_VALIDATION = "NOT_EXTERNALLY_VALIDATED_GAMEPLAY"


@dataclass(frozen=True)
class Womb2LifecycleResult:
    completed: bool
    generation_status: str
    external_validation: str
    layout: Womb2FullResult
    post_layout: PostLayoutLifecycleResult
    next_run_state: CanonicalRunGenerationSnapshot


def generate_womb2_lifecycle(
    womb1_result: Womb1LifecycleResult,
    *,
...
) -> Womb2LifecycleResult:
    layout = generate_womb2_full(
        womb1_result.layout,
        womb1_result.next_run_state,
    ...
    if not layout.completed:
        raise RuntimeError("Womb II layout did not complete within max_attempts")
    ...
    pool_states = list(womb1_result.next_run_state.boss_pool_rng_states)
    ...
    next_state = CanonicalRunGenerationSnapshot(
        ...
        lifecycle="womb2-success-tail-complete",
    )
    return Womb2LifecycleResult(
        True,
        WOMB2_LIFECYCLE_STATUS,
        EXTERNAL_VALIDATION,
        layout,
        tail,
        next_state,
