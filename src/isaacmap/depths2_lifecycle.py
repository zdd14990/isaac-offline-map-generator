"""Canonical Depths-II accepted layout plus successful lifecycle handoff.

The Stage-6 post-layout tail reuses the shared ``place_post_topology_rooms``
tail (RVA ``0x0033D1D6..0x0033D94B``).  Stage 6 (room-config stage 7) skips
...
from .depths1_lifecycle import Depths1LifecycleResult
from .depths2_full_pipeline import Depths2FullResult, generate_depths2_full
...
DEPTHS2_LIFECYCLE_STATUS = "DEPTHS2_CANONICAL_LIFECYCLE_CONFIRMED_BINARY"
EXTERNAL_VALIDATION = "NOT_EXTERNALLY_VALIDATED_GAMEPLAY"


@dataclass(frozen=True)
class Depths2LifecycleResult:
    completed: bool
    generation_status: str
    external_validation: str
    layout: Depths2FullResult
    post_layout: PostLayoutLifecycleResult
    next_run_state: CanonicalRunGenerationSnapshot


def generate_depths2_lifecycle(
    depths1_result: Depths1LifecycleResult,
    *,
...
) -> Depths2LifecycleResult:
    layout = generate_depths2_full(
        depths1_result.layout,
        depths1_result.next_run_state,
    ...
    if not layout.completed:
        raise RuntimeError("Depths II layout did not complete within max_attempts")
    ...
    pool_states = list(depths1_result.next_run_state.boss_pool_rng_states)
    ...
    next_state = CanonicalRunGenerationSnapshot(
        ...
        lifecycle="depths2-success-tail-complete",
    )
    return Depths2LifecycleResult(
        True,
        DEPTHS2_LIFECYCLE_STATUS,
        EXTERNAL_VALIDATION,
        layout,
        tail,
        next_state,
