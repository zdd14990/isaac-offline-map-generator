"""Canonical Dross (2+) accepted-layout research composition.

Stage-2 ALT entry: StageType 5, rcs 28, slot 3, pool 28.  The first-half
mandatory end room (0x0033C416..0x0033C4D1) fires on every Dross floor.
...
from .boss_pool import DROSS_POOL_INDEX, BossPoolEntry, BossPoolRuntimeState
from .floor_init import derive_alt_topology_inputs
from .generation_state import (
    CanonicalDrossProfile,
...
DROSS_LAYOUT_STATUS = "DROSS_FULL_LAYOUT_PIPELINE_PARTIAL_BINARY"
EXTERNAL_VALIDATION = "NOT_EXTERNALLY_VALIDATED_GAMEPLAY"
LEVEL_SHIFT_INDEX = 35


def generate_dross_full(
    start_seed: int,
    difficulty: str,
    *,
    ...
    """Generate Dross (ALT stage 2, rcs 28) from the run seed."""
    inputs = derive_alt_topology_inputs(2, 5, start_seed, difficulty)
    profile = CanonicalDrossProfile(
        difficulty,
    ...
    entries = (
        ...
        + entries_from_definitions(dross_definitions, stage=28)
    )
    run_state = RunGenerationState(
        start_seed,
        BossPoolRuntimeState.fresh_alt(start_seed, DROSS_POOL_INDEX, dross_boss_entries),
    ...
    for attempt_number in range(1, max_attempts + 1):
        ...
        start_boss = run_state.boss_pools.pool_rngs[DROSS_POOL_INDEX].seed
            ...
            if not m5.completed:
                ...
                if not m6.completed:
                    ...
                    m6_attempt = SecretGenerationAttemptResult(
                        ...
                        generator.rng.seed,
                        start_boss,
                        run_state.boss_pools.pool_rngs[DROSS_POOL_INDEX].seed,
                        used_before,
                    ...
                    m7 = run_secret_to_ultra(
                        _m6_boundary(
                            ...
                            level_rng=level_rng,
                            generator=generator,
                            boss_pool_index=DROSS_POOL_INDEX,
                        ),
                        entries=entries,
                    )
        ...
        if late is not None and late.completed:
            ...
            run_state.current_stage_lifecycle = "dross-full-layout-accepted"
