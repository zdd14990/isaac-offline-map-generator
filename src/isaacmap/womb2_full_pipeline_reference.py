"""Mechanical Womb-II accepted-layout reference composition.

Independent orchestration reference.  It replays the Womb-I reference
(which itself replays Basement..Depths-II) through the successful tail, then
composes the already binary-confirmed reference leaves through the Stage-8
attempt transaction.  The Womb BossPool (index 10) continues from its
post-Womb-I state and is advanced by the Womb-II selection.
"""
...
from .floor_init_reference import derive_womb2_topology_inputs_reference
from .generation_state import CanonicalWomb2Profile
...
from .womb1_full_pipeline_reference import generate_womb1_full_reference


def generate_womb2_full_reference(
    start_seed: int,
    difficulty: str,
    *,
    ...
    w1 = generate_womb1_full_reference(
        start_seed,
        difficulty,
    ...
    if not w1.completed:
        raise RuntimeError("reference Womb I layout did not complete")
    ...
    w1_tail = run_post_layout_lifecycle_reference(
        level_rng_state=w1.final_level,
        entries=special + depths + womb + blue_womb,
        room_config_weights=w1.weights,
        special_room_used_bits=w1.used_bits,
        level_stage=7,
        room_config_stage=10,
    )

    inputs = derive_womb2_topology_inputs_reference(start_seed, difficulty)
    profile = CanonicalWomb2Profile(
        difficulty,
        effective_curse_mask=inputs.curse_mask,
        is_xl=inputs.is_xl,
        special_room_used_bits=frozenset(w1_tail.used_bits),
    )
    profile.validate()

    entries = special + basement + caves + depths + womb + blue_womb
    weights = RoomConfigMutableStateReference(dict(w1_tail.weights))
    weights.reset_pool(womb)

    bosses = BossPoolRuntimeReference.resume_canonical_womb(
        start_seed,
        womb_boss_entries,
        pool_state=w1.final_boss,
        removed=w1.removed,
    )
    ...
    used_bits = set(w1_tail.used_bits)
