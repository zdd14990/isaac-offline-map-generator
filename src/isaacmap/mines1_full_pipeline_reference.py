"""Mechanical Mines1 (2+) accepted-layout reference composition.

Independent orchestration reference for the Stage-2 ALT entry (stage_type 5,
rcs 28, slot 4).  It never calls a clean high-level pipeline.
"""
...
from .generation_state import CanonicalMines1Profile
...
def generate_mines1_full_reference(
    start_seed: int,
    difficulty: str,
    *,
    mines1_boss_entries: tuple[BossPoolEntryReference, ...],
    special_definitions: tuple[RoomDefinition, ...],
    mines1_definitions: tuple[RoomDefinition, ...],
    max_attempts: int = 100,
) -> ReferenceBasement1FullResult:
    inputs = derive_alt_topology_inputs_reference(3, 4, start_seed, difficulty)
    ...
    entries = (
        ...
        + entries_from_definitions_reference(mines1_definitions, stage=29)
    )
    ...
    for number in range(1, max_attempts + 1):
        ...
        start_boss = bosses.pool_states[MINES_POOL_INDEX]
            ...
            if not m5.completed:
                ...
                if not m6.completed:
                    ...
                    m6_attempt = ReferenceSecretAttempt(
                        ...
                        generator.rng.seed,
                        start_boss,
                        bosses.pool_states[MINES_POOL_INDEX],
                        used_before,
