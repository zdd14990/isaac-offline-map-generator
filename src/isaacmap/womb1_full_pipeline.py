"""Canonical Womb-I accepted-layout research composition.

All geometry and selection work is delegated to existing clean leaves.  This
module owns only the Stage-7 replay/attempt transaction and does not perform
the successful post-layout lifecycle handoff to later floors.

Evidence grade: PARTIAL_BINARY.  The stage-7 branch gates (Boss, Type8,
Shop, Treasure, Planetarium-skip, Secret, Ultra, late defaults) are
statically reconstructed; the validation corpus is intentionally limited
(50 NORMAL + 50 HARD clean-vs-reference comparisons).
"""
...
from .floor_init import derive_womb1_topology_inputs
from .generation_state import (
    CanonicalWomb1Profile,
...
WOMB1_LAYOUT_STATUS = "WOMB1_FULL_LAYOUT_PIPELINE_PARTIAL_BINARY"
EXTERNAL_VALIDATION = "NOT_EXTERNALLY_VALIDATED_GAMEPLAY"
LEVEL_SHIFT_INDEX = 35


@dataclass(frozen=True)
class Womb1FullResult:
    completed: bool
    ...
    difficulty: str
    stage_seed: int
    replayed_depths2: Depths2FullResult
...
def generate_womb1_full(
    depths2_result: Depths2FullResult,
    ...
    *,
    ...
    """Generate Womb I from a successful Depths-II result and explicit entry state.

    The entry state must be a CanonicalRunGenerationSnapshot with
    next_level_stage=7 and completed_level_stage=6, typically from
    Depths2LifecycleResult.next_run_state.  The Womb BossPool (index 10) is
    fresh from its run-start state (no earlier floor selects it).
    """
    from .generation_state import CanonicalRunGenerationSnapshot

    snapshot: CanonicalRunGenerationSnapshot = entry_state_snapshot
    if snapshot.next_level_stage != 7 or snapshot.completed_level_stage != 6:
        raise ValueError(
            f"Womb I entry requires snapshot with "
            f"next_level_stage=7, completed_level_stage=6; "
            f"got next={snapshot.next_level_stage}, completed={snapshot.completed_level_stage}"
        )

    start_seed = snapshot.game_start_seed

    inputs = derive_womb1_topology_inputs(start_seed, depths2_result.difficulty)
    profile = CanonicalWomb1Profile(
        depths2_result.difficulty,
    ...
    womb_entries = entries_from_definitions(womb_definitions, stage=10)
    ...
    room_config.reset(womb_entries)
    ...
    pool_rngs[WOMB_POOL_INDEX].seed = snapshot.boss_pool_rng_states[WOMB_POOL_INDEX]
    ...
    bosses = BossPoolRuntimeState(
        ...
        pool_rngs=pool_rngs,
        shuffled_entries={
            WOMB_POOL_INDEX: shuffle_entries(womb_boss_entries, seeds[WOMB_POOL_INDEX]),
        },
    ...
    run_state = RunGenerationState(
        ...
        bosses,
        room_config,
        "depths2-success-tail-complete/womb1-entered",
    ...
    for attempt_number in range(1, max_attempts + 1):
        ...
        start_boss = run_state.boss_pools.pool_rngs[WOMB_POOL_INDEX].seed
            ...
            if not m5.completed:
                ...
                if not m6.completed:
                    ...
                    m6_attempt = SecretGenerationAttemptResult(
                        ...
                        generator.rng.seed,
                        start_boss,
                        run_state.boss_pools.pool_rngs[WOMB_POOL_INDEX].seed,
                        used_before,
                    ...
                    m7 = run_secret_to_ultra(
                        _m6_boundary(
                            ...
                            difficulty=depths2_result.difficulty,
        ...
        if late is not None and late.completed:
            ...
            run_state.current_stage_lifecycle = "womb1-full-layout-accepted"
            return Womb1FullResult(
                True,
                WOMB1_LAYOUT_STATUS,
                EXTERNAL_VALIDATION,
                start_seed,
