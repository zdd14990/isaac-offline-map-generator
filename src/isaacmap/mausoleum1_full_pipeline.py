"""Canonical Mausoleum1 (2+) accepted-layout research composition.

Stage-5 ALT entry: StageType 4, rcs 31, slot 6, pool 31.  The first-half
mandatory end room (0x0033C416..0x0033C4D1) fires on every Mausoleum1 floor.
...
from .boss_pool import MAUSOLEUM_POOL_INDEX, BossPoolEntry, BossPoolRuntimeState
from .floor_init import derive_alt_topology_inputs
from .generation_state import (
    CanonicalMausoleum1Profile,
...
MAUSOLEUM1_LAYOUT_STATUS = "MAUSOLEUM1_FULL_LAYOUT_PIPELINE_PARTIAL_BINARY"
EXTERNAL_VALIDATION = "NOT_EXTERNALLY_VALIDATED_GAMEPLAY"
LEVEL_SHIFT_INDEX = 35


def generate_mausoleum1_full(
    start_seed: int,
    difficulty: str,
    *,
    ...
    """Generate Mausoleum1 (ALT stage 2, rcs 28) from the run seed."""
    inputs = derive_alt_topology_inputs(5, 4, start_seed, difficulty)
    profile = CanonicalMausoleum1Profile(
        difficulty,
    ...
    entries = (
        ...
        + entries_from_definitions(mausoleum1_definitions, stage=31)
    )
    run_state = RunGenerationState(
        start_seed,
        BossPoolRuntimeState.fresh_alt(start_seed, MAUSOLEUM_POOL_INDEX, mausoleum1_boss_entries),
    ...
    for attempt_number in range(1, max_attempts + 1):
        ...
        start_boss = run_state.boss_pools.pool_rngs[MAUSOLEUM_POOL_INDEX].seed
            ...
            if not m5.completed:
                ...
                if not m6.completed:
                    ...
                    m6_attempt = SecretGenerationAttemptResult(
                        ...
                        generator.rng.seed,
                        start_boss,
                        run_state.boss_pools.pool_rngs[MAUSOLEUM_POOL_INDEX].seed,
                        used_before,
                    ...
                    m7 = run_secret_to_ultra(
                        _m6_boundary(
                            ...
                            level_rng=level_rng,
                            generator=generator,
                            boss_pool_index=MAUSOLEUM_POOL_INDEX,
                        ),
                        entries=entries,
                    )
        ...
        if late is not None and late.completed:
            ...
            run_state.current_stage_lifecycle = "mausoleum1-full-layout-accepted"
            return Basement1FullResult(
                True,
                MAUSOLEUM1_LAYOUT_STATUS,
                EXTERNAL_VALIDATION,
                start_seed,
                difficulty,
                inputs.stage_seed,
