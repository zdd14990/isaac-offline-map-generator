"""Mechanical Depths-I accepted-layout reference composition.

Independent orchestration reference.  It composes the already binary-confirmed
reference leaves through the Stage-5 attempt transaction.  The Depths BossPool
(index 7) is seeded from ``bosspools.xml["depths"]`` and starts from its
run-start state (no earlier floor selects it).
"""
...
from .caves2_full_pipeline_reference import generate_caves2_full_reference
from .floor_init_reference import derive_depths1_topology_inputs_reference
from .generation_state import CanonicalDepths1Profile
...
def generate_depths1_full_reference(
    start_seed: int,
    difficulty: str,
    *,
    ...
    c2 = generate_caves2_full_reference(
        start_seed,
        difficulty,
    ...
    if not c2.completed:
        raise RuntimeError("reference Caves II layout did not complete")
    ...
    # Caves II successful tail -> Depths I entry state (weights/used-bits).
    c2_tail = run_post_layout_lifecycle_reference(
        level_rng_state=c2.final_level,
        entries=special + caves + blue_womb,
        room_config_weights=c2.weights,
        special_room_used_bits=c2.used_bits,
        level_stage=4,
        room_config_stage=4,
    )

    inputs = derive_depths1_topology_inputs_reference(start_seed, difficulty)
    profile = CanonicalDepths1Profile(
        difficulty,
        effective_curse_mask=inputs.curse_mask,
        is_xl=inputs.is_xl,
        special_room_used_bits=frozenset(c2_tail.used_bits),
    )
    profile.validate()

    entries = special + basement + caves + depths + blue_womb
    weights = RoomConfigMutableStateReference(dict(c2_tail.weights))
    ...
    bosses = BossPoolRuntimeReference.resume_canonical_depths(
        start_seed,
        depths_boss_entries,
        pool_state=seeds[DEPTHS_POOL_INDEX],
        removed=c2.removed,
    )
