"""Canonical Downpour-I accepted-layout research composition.

All geometry and selection work is delegated to the existing clean leaves.
This module owns only the Stage-1 ALT (stage_type 4, rcs 27) transaction.

Evidence grade: PARTIAL_BINARY.  The ALT special-room gates
(adjusted-stage MiniBoss/Challenge/Chest) are reconstructed from the binary;
the validation corpus is intentionally limited (50 NORMAL + 50 HARD
clean-vs-reference comparisons).
"""
...
DOWNPOUR1_LAYOUT_STATUS = "DOWNPOUR1_FULL_LAYOUT_PIPELINE_PARTIAL_BINARY"
EXTERNAL_VALIDATION = "NOT_EXTERNALLY_VALIDATED_GAMEPLAY"
LEVEL_SHIFT_INDEX = 35


def generate_downpour1_full(
    start_seed: int,
    difficulty: str,
    *,
    ...
    """Generate Downpour I (ALT stage 1, rcs 27) from the run seed."""
    inputs = derive_alt_topology_inputs(1, 4, start_seed, difficulty)
    ...
    entries = (
        ...
        + entries_from_definitions(downpour_definitions, stage=27)
    )
    run_state = RunGenerationState(
        start_seed,
        BossPoolRuntimeState.fresh_alt(
    ...
    for attempt_number in range(1, max_attempts + 1):
        ...
        start_boss = run_state.boss_pools.pool_rngs[DOWNPOUR_POOL_INDEX].seed
            ...
            if not m5.completed:
                ...
                if not m6.completed:
                    ...
                    m6_attempt = SecretGenerationAttemptResult(
                        ...
                        generator.rng.seed,
                        start_boss,
                        run_state.boss_pools.pool_rngs[DOWNPOUR_POOL_INDEX].seed,
                        used_before,
                    ...
                    m7 = run_secret_to_ultra(
                        _m6_boundary(
                            ...
                            level_rng=level_rng,
                            generator=generator,
                            boss_pool_index=DOWNPOUR_POOL_INDEX,
                        ),
                        entries=entries,
                    )
        ...
        if late is not None and late.completed:
            ...
            run_state.current_stage_lifecycle = "downpour1-full-layout-accepted"
            return Basement1FullResult(
                True,
                DOWNPOUR1_LAYOUT_STATUS,
                EXTERNAL_VALIDATION,
                start_seed,
                difficulty,
                inputs.stage_seed,
