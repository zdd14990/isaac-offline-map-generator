"""Offline Womb-I clean/reference differential; never runs the game."""
...
def main() -> int:
    ...
    try:
        for seed in range(args.start, args.start + args.count):
            ...
            stats["comparisons"] += 1
            attempts = len(w1.layout.attempts)
            stats["attempts"] += attempts
            stats["retrying_seeds"] += int(attempts > 1)
            stats["max_attempts"] = max(stats["max_attempts"], attempts)
            stats["rng_transitions"] += len(w1.post_layout.rng_transitions)
            stats["room_config_selections"] += len(w1.post_layout.assignments)
            stats["boss_selections"] += 1
            stats["secret_placements"] += 1
            stats["ultra_placements"] += 1
            if any(attempt.is_xl for attempt in w1.layout.attempts if hasattr(attempt, "is_xl")) or \
               getattr(w1.layout, "is_xl", False):
                stats["xl_floors"] += 1
