"""Offline Downpour-I clean/reference differential; never runs the game."""
...
def main() -> int:
    ...
    downpour_bosses = pools["downpour"]
    downpour_refs = _refs(downpour_bosses)

    special = tuple(load_stb(args.resources / "rooms/00.special rooms.stb"))
    downpour = tuple(load_stb(args.resources / "rooms/27.downpour.stb"))
    ...
    try:
        for seed in range(args.start, args.start + args.count):
            ...
            reference = generate_downpour1_full_reference(
                seed,
                args.difficulty,
                downpour_boss_entries=downpour_refs,
                special_definitions=special,
                downpour_definitions=downpour,
            )
            ...
            if clean.layout.attempts[clean.layout.accepted_attempt_index].topology_usable and \
               any(hasattr(a, "is_xl") for a in ()) or getattr(clean.layout, "is_xl", False):
                pass
