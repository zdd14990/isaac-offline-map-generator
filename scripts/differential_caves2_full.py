"""Offline Caves-II clean/reference differential; never runs the game."""
    ...
    try:
        for seed in range(args.start, args.start + args.count):
            ...
            reference = generate_caves2_full_reference(
                seed,
                args.difficulty,
                basement_boss_entries=basement_refs,
                caves_boss_entries=caves_refs,
                special_definitions=special,
                basement_definitions=basement,
                caves_definitions=caves,
                blue_womb_definitions=blue_womb,
            ...
            stats["comparisons"] += 1
            attempts = len(c2.layout.attempts)
            stats["attempts"] += attempts
            stats["retrying_seeds"] += int(attempts > 1)
            stats["max_attempts"] = max(stats["max_attempts"], attempts)
            stats["rng_transitions"] += len(c2.post_layout.rng_transitions)
            stats["room_config_selections"] += len(c2.post_layout.assignments)
