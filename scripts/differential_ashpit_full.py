"""Offline Ashpit (2+) clean/reference differential; never runs the game."""
...
def main() -> int:
    ...
    ashpit_bosses = pools["ashpit"]
    ashpit_refs = _refs(ashpit_bosses)

    special = tuple(load_stb(args.resources / "rooms/00.special rooms.stb"))
    ashpit = tuple(load_stb(args.resources / "rooms/30.ashpit.stb"))
    ...
    try:
        for seed in range(args.start, args.start + args.count):
            ...
            reference = generate_ashpit_full_reference(
                seed,
                args.difficulty,
                ashpit_boss_entries=ashpit_refs,
                special_definitions=special,
                ashpit_definitions=ashpit,
            )
