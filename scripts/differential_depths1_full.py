"""Offline Depths-I clean/reference differential; never runs the game."""
...
def main() -> int:
    ...
    try:
        for seed in range(args.start, args.start + args.count):
            ...
            is_xl = derive_depths1_topology_inputs(seed, args.difficulty).is_xl
            stats["comparisons"] += 1
            attempts = len(d1.layout.attempts)
            stats["attempts"] += attempts
            stats["retrying_seeds"] += int(attempts > 1)
            stats["max_attempts"] = max(stats["max_attempts"], attempts)
            stats["rng_transitions"] += len(d1.post_layout.rng_transitions)
            stats["room_config_selections"] += len(d1.post_layout.assignments)
