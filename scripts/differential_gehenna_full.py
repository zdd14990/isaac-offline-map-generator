"""Offline Gehenna (2+) clean/reference differential; never runs the game."""
...
def main() -> int:
    ...
    gehenna_bosses = pools["gehenna"]
    gehenna_refs = _refs(gehenna_bosses)

    special = tuple(load_stb(args.resources / "rooms/00.special rooms.stb"))
    gehenna = tuple(load_stb(args.resources / "rooms/32.gehenna.stb"))
    ...
    try:
        for seed in range(args.start, args.start + args.count):
            ...
            reference = generate_gehenna_full_reference(
                seed,
                args.difficulty,
                gehenna_boss_entries=gehenna_refs,
                special_definitions=special,
                gehenna_definitions=gehenna,
            )
