"""Offline Mausoleum1 (2+) clean/reference differential; never runs the game."""
...
def main() -> int:
    ...
    mausoleum1_bosses = pools["mausoleum"]
    mausoleum1_refs = _refs(mausoleum1_bosses)

    special = tuple(load_stb(args.resources / "rooms/00.special rooms.stb"))
    mausoleum1 = tuple(load_stb(args.resources / "rooms/31.mausoleum.stb"))
    ...
    try:
        for seed in range(args.start, args.start + args.count):
            ...
            reference = generate_mausoleum1_full_reference(
                seed,
                args.difficulty,
                mausoleum1_boss_entries=mausoleum1_refs,
                special_definitions=special,
                mausoleum1_definitions=mausoleum1,
            )
