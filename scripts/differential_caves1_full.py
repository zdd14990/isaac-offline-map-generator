"""Offline Caves-I clean/reference differential; never runs the game."""
...
def main() -> int:
    ...
    try:
        for seed in range(args.start, args.start + args.count):
            ...
            stats["comparisons"] += 1
            attempts = len(c1.layout.attempts)
            stats["attempts"] += attempts
            stats["retrying_seeds"] += int(attempts > 1)
            stats["max_attempts"] = max(stats["max_attempts"], attempts)
            stats["rng_transitions"] += len(c1.post_layout.rng_transitions)
            stats["room_config_selections"] += len(c1.post_layout.assignments)
            is_xl = derive_caves1_topology_inputs(seed, args.difficulty).is_xl
            stats["boss_selections"] += 1 + int(is_xl)
            stats["xl_floors"] += int(is_xl)
