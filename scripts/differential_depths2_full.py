"""Offline Depths-II clean/reference differential; never runs the game."""
    ...
    try:
        for seed in range(args.start, args.start + args.count):
            ...
            reference = generate_depths2_full_reference(
                ...
                depths_boss_entries=depths_refs,
                special_definitions=special, basement_definitions=basement,
                caves_definitions=caves, depths_definitions=depths,
                blue_womb_definitions=blue_womb,
            )
            ...
            stats["comparisons"] += 1
            attempts = len(d2.layout.attempts)
            stats["attempts"] += attempts
            stats["retrying_seeds"] += int(attempts > 1)
            stats["max_attempts"] = max(stats["max_attempts"], attempts)
            stats["rng_transitions"] += len(d2.post_layout.rng_transitions)
            stats["room_config_selections"] += len(d2.post_layout.assignments)
