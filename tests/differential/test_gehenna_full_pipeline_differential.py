...
from isaacmap.boss_pool import GEHENNA_POOL_INDEX, load_boss_pool_xml
from isaacmap.boss_pool_reference import BossPoolEntryReference
from isaacmap.gehenna_full_pipeline_reference import generate_gehenna_full_reference
from isaacmap.gehenna_lifecycle import generate_gehenna_lifecycle
...
GEHENNA_BOSS_REFS = _refs(GEHENNA_BOSSES)
TAIL_REF_ENTRIES = (
...
def test_gehenna_clean_matches_reference(seed: int, difficulty: str) -> None:
    clean = generate_gehenna_lifecycle(
        seed,
        difficulty,
        gehenna_boss_entries=GEHENNA_BOSSES,
        special_definitions=SPECIAL,
        gehenna_definitions=GEHENNA,
        blue_womb_definitions=BLUE_WOMB,
    )

    reference = generate_gehenna_full_reference(
        seed,
        difficulty,
        gehenna_boss_entries=GEHENNA_BOSS_REFS,
        special_definitions=SPECIAL,
        gehenna_definitions=GEHENNA,
    )
    ...
    assert clean.next_run_state.completed_level_stage == 6
    assert clean.next_run_state.next_level_stage == 7
    assert clean.next_run_state.boss_pool_rng_states[GEHENNA_POOL_INDEX] == reference.final_boss


@pytest.mark.parametrize("difficulty", ("NORMAL", "HARD"))
def test_gehenna_mandatory_end_room_block_fires(difficulty: str) -> None:
    """Gehenna is outside both first/second-half stage sets (1..4), so the
    alternate-path end room predicates never fire; the M6 leaf still records
    the branch evaluation as an AltEndRoom block with config None.  Gehenna's
    boss is FIXED (Mom-Mausoleum 89) via the select_boss_id level-6 switch."""
    clean = generate_gehenna_lifecycle(
        1,
        difficulty,
        gehenna_boss_entries=GEHENNA_BOSSES,
