"""True clean-vs-reference differential for the Caves II accepted layout + tail."""
...
from isaacmap.caves2_full_pipeline_reference import generate_caves2_full_reference
...
def _caves1(seed: int, difficulty: str):
    ...
    return generate_caves1_lifecycle(
        b2,
...
def test_caves2_clean_matches_reference(seed: int, difficulty: str) -> None:
    c1 = _caves1(seed, difficulty)
    ...
    reference = generate_caves2_full_reference(
        seed,
        difficulty,
    ...
    compare_basement1_full(c2.layout, reference)
    ...
    compare_post_layout_lifecycle(c2.post_layout, reference_tail)

    assert c2.next_run_state.completed_level_stage == 4
    assert c2.next_run_state.next_level_stage == 5
    assert c2.next_run_state.boss_pool_rng_states[CAVES_POOL_INDEX] == reference.final_boss
