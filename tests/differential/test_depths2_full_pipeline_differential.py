"""True clean-vs-reference differential for the Depths II accepted layout + tail."""
...
from isaacmap.depths2_full_pipeline_reference import generate_depths2_full_reference
...
def _depths1(seed: int, difficulty: str):
    ...
    return generate_depths1_lifecycle(
...
def test_depths2_clean_matches_reference(seed: int, difficulty: str) -> None:
    d1 = _depths1(seed, difficulty)
    ...
    reference = generate_depths2_full_reference(
        seed,
        difficulty,
    ...
    compare_basement1_full(d2.layout, reference)
    ...
    compare_post_layout_lifecycle(d2.post_layout, reference_tail)

    assert d2.next_run_state.completed_level_stage == 6
    assert d2.next_run_state.next_level_stage == 7
    assert d2.next_run_state.boss_pool_rng_states[DEPTHS_POOL_INDEX] == reference.final_boss


@pytest.mark.parametrize("difficulty", ("NORMAL", "HARD"))
def test_depths2_never_xl(difficulty: str) -> None:
    for seed in range(1, 10_001):
        assert not derive_depths2_topology_inputs(seed, difficulty).is_xl
