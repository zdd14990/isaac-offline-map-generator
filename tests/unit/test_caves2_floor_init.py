...
from isaacmap.floor_init import derive_caves2_topology_inputs
from isaacmap.floor_init_reference import derive_caves2_topology_inputs_reference
...
def test_caves2_initialization_matches_independent_reference(difficulty: str) -> None:
    masks: set[int] = set()
    for seed in range(1, 10_001):
        clean = derive_caves2_topology_inputs(seed, difficulty)
        reference = derive_caves2_topology_inputs_reference(seed, difficulty)
...
@pytest.mark.parametrize("difficulty", ("NORMAL", "HARD"))
def test_caves2_stage_constants(difficulty: str) -> None:
    inputs = derive_caves2_topology_inputs(1, difficulty)
    assert inputs.room_config_stage == 4
    assert inputs.required_dead_ends == 6
    assert inputs.allowed_shapes_mask == 0x1FFE
    assert not inputs.is_xl
    assert inputs.target_room_count in (18, 19, 20, 22, 23, 24)
...
def test_caves2_rejects_unsupported_difficulty() -> None:
    with pytest.raises(ValueError, match="NORMAL or HARD"):
        derive_caves2_topology_inputs(1, "GREED")
