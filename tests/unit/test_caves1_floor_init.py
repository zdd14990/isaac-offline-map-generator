...
def test_caves1_initialization_matches_independent_reference(difficulty: str) -> None:
    masks: set[int] = set()
    saw_xl = False
    for seed in range(1, 10_001):
        clean = derive_caves1_topology_inputs(seed, difficulty)
        reference = derive_caves1_topology_inputs_reference(seed, difficulty)
...
@pytest.mark.parametrize("difficulty", ("NORMAL", "HARD"))
def test_caves1_natural_xl_changes_topology_contract(difficulty: str) -> None:
    xl = next(
        derive_caves1_topology_inputs(seed, difficulty)
        for seed in range(1, 10_001)
        if derive_caves1_topology_inputs(seed, difficulty).is_xl
    )
    assert xl.effective_curse_mask == 2
    assert xl.required_dead_ends == 7
    assert xl.target_room_count >= 27 + (2 if difficulty == "HARD" else 0)
    assert (xl.room_config_min_difficulty, xl.room_config_max_difficulty) == (
        (5, 15) if difficulty == "HARD" else (1, 10)
    )


def test_caves1_non_xl_stage_constants() -> None:
    normal = next(
        derive_caves1_topology_inputs(seed, "NORMAL")
        for seed in range(1, 100)
        if not derive_caves1_topology_inputs(seed, "NORMAL").is_xl
    )
    hard = next(
        derive_caves1_topology_inputs(seed, "HARD")
        for seed in range(1, 100)
        if not derive_caves1_topology_inputs(seed, "HARD").is_xl
    )
    assert normal.room_config_stage == hard.room_config_stage == 4
...
def test_caves1_rejects_unsupported_difficulty() -> None:
    with pytest.raises(ValueError, match="NORMAL or HARD"):
        derive_caves1_topology_inputs(1, "GREED")
